...
namespace DATABASE {
	...
	bool unionKeyMeta::init(const uint16_t *columnIndexs, uint16_t columnCount, const META::tableMeta* meta)
	{
		if (m_types)
			delete[]m_types;
		m_types = new META::COLUMN_TYPE[columnCount];
		m_fixed = true;
		for (uint16_t i = 0; i < columnCount; i++)
		{
			m_types[i] = meta->getColumn(columnIndexs[i])->m_columnType;
			if (!META::columnInfos[static_cast<int>(m_types[i])].asIndex)
			{
				delete[]m_types;
				m_types = nullptr;
				return false;
			}
			if (!META::columnInfos[static_cast<int>(m_types[i])].fixed)
			{
				if (m_fixed)
					m_fixed = false;
				...
				m_size += META::columnInfos[static_cast<int>(m_types[i])].columnTypeSize;
		}
	...
	int unionKey::compare(const unionKey & dest) const
	{
		...
		for (uint16_t i = 0; i < meta->m_keyCount; i++)
		{
			...
			if (META::columnInfos[static_cast<int>(meta->m_types[i])].fixed)
			{
				srcKey += META::columnInfos[static_cast<int>(meta->m_types[i])].columnTypeSize;
				destKey += META::columnInfos[static_cast<int>(meta->m_types[i])].columnTypeSize;
			}
			else
			{
	...
	*/
	const char* unionKey::initKey(leveldb::Arena * arena, unionKeyMeta * keyMeta, uint16_t *columnIdxs, uint16_t columnCount, const DATABASE_INCREASE::DMLRecord * r, bool keyUpdated)
	{
		...
		if (!keyMeta->m_fixed)
		{
			...
			{
				if (!META::columnInfos[static_cast<int>(keyMeta->m_types[idx])].fixed)
				{
					if (keyUpdated)
			...
			((r->head->minHead.type == DATABASE_INCREASE::R_UPDATE || r->head->minHead.type == DATABASE_INCREASE::R_REPLACE) && !keyUpdated))
		{
			...
			{
				if (META::columnInfos[static_cast<int>(keyMeta->m_types[i])].fixed)
				{
					memcpy(ptr, r->column(columnIdxs[i]), META::columnInfos[static_cast<int>(keyMeta->m_types[i])].columnTypeSize);
					ptr += META::columnInfos[static_cast<int>(keyMeta->m_types[i])].columnTypeSize;
				}
				else
				{
