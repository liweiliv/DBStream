...
public:
	...
	inline void* set(uint32_t id, void * data)
	{
		...
		if (mid == nullptr)
		{
			...
			while (!root.child[highId].compare_exchange_weak(nullData, mid,std::memory_order_release, std::memory_order_relaxed))
			{
				if (nullData != nullptr)
				{
					delete mid;
		...
		if (low == nullptr)
		{
			...
			while (!mid->child[midId].compare_exchange_weak(nullData, low,std::memory_order_release, std::memory_order_relaxed))
			{
				if (nullData != nullptr)
				{
					delete low;
		...
		if ((tmpData = low->child[lowId].load(std::memory_order_relaxed))
				!= nullptr)
			return tmpData;
		while (!low->child[lowId].compare_exchange_weak(tmpData, data,std::memory_order_release, std::memory_order_relaxed))
		{
			if (tmpData != nullptr)
				return tmpData;
		}
		...
		do
		{
			...
			if (_min <= id)
				break;
			if (min.compare_exchange_weak(_min, id, std::memory_order_release,
		...
		do
		{
			...
			if (_max >= id)
				break;
			if (max.compare_exchange_weak(_max, id, std::memory_order_release,
					std::memory_order_relaxed))
				break;
		} while (1);
		return data;
	...
	inline void purge(uint32_t id)
	{
		...
		{
			lowNode * low;
			if ((low = mid->child[j].load(std::memory_order_release)) != NULL)
			{
				mid->child[j].store(nullptr, std::memory_order_release);
				for (uint16_t m = 0; m < PT_LOW(0xffffffffu); m++)
				{
					void * data;
					if ((data = low->child[m].load(std::memory_order_release))
							!= NULL)
					{
						if (destoryValueFunc)
							destoryValueFunc(data);
		...
		{
			void * data;
			if ((data = low->child[m].load(std::memory_order_release)) != NULL)
			{
