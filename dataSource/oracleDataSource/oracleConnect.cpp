...
{
		oracleConnect::oracleConnect(config* conf) :m_env(nullptr), m_isScanIp(false)
		{
		...
		dsStatus& oracleConnect::connect(oracle::occi::Connection*& conn, const std::string& sid, const std::string& serviceName)
		{
			conn = nullptr;
			std::string errInfo;
		...
		void oracleConnect::closeStmt(oracle::occi::Connection* conn, oracle::occi::Statement*& stmt, oracle::occi::ResultSet*& rs)
		{
			if (rs != nullptr)
			{
		...
		void oracleConnect::close(oracle::occi::Connection*& conn)
		{
			if (conn != nullptr)
			{
		...
		void oracleConnect::close(oracle::occi::Connection*& conn, oracle::occi::Statement*& stmt, oracle::occi::ResultSet*& rs)
		{
			closeStmt(conn, stmt, rs);
			close(conn);
		}

		bool oracleConnect::isRac()
		{
			return m_allNodes.size() > 1;
		}

		dsStatus& oracleConnect::init()
		{
		...
		dsStatus& oracleConnect::connect(oracle::occi::Connection*& conn)
		{
			dsReturn(connect(conn, m_sid, m_serviceName));
		}

		dsStatus& oracleConnect::connectByNodeId(int threadId, oracle::occi::Connection*& conn)
		{
		...
		bool oracleConnect::errorRecoverable(int errorCode)
		{
			switch (errorCode)
			{
			case 28:
