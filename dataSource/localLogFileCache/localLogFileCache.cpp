...
namespace DATA_SOURCE
{
	...
	DS LocalLogFileCache::flush(logFile* begin, bool flushAll, bool flushIndex)
	{
	...
	void LocalLogFileCache::flushAndPurgeThread()
	{
	...
	int64_t LocalLogFileCache::getLogFileId(const char* name)
	{
		int64_t id = 0;
		const char* p = name;
		while (*p <= '9' && *p >= '0')
	...
	DLL_EXPORT LocalLogFileCache::LocalLogFileCache(Config* conf) :m_current(nullptr), m_begin(nullptr), m_firstUnFlushed(nullptr), m_firstNotNeedPurged(nullptr),
	...
	DLL_EXPORT LocalLogFileCache::~LocalLogFileCache()
	{
		while (m_begin != nullptr)
		{
			if (!m_begin->isFlushed())
				m_begin->flush(m_baseDir.c_str(),m_compressBuf,m_compressBufSize);
			logFile* next = m_begin->getNext();
			delete m_begin;
			m_begin = next;
		}
	}
	...
	DS LocalLogFileCache::loadFiles(std::set<int64_t>& fileIds)
	{
#if defined  OS_WIN
	...
	DLL_EXPORT DS LocalLogFileCache::init(uint64_t purgedSeqNo)
	{
		RPC::LogSeqNo purgedLogSeqNo;
		purgedLogSeqNo.seqNo = purgedSeqNo;
		std::set<int64_t> fileIds;
		dsReturnIfFailed(loadFiles(fileIds));
		if (fileIds.empty())
		{
		...
		for (logFile* f = m_begin; f != nullptr; f = f->getNext())
		{
			if (!m_current->getMetaInfo().getFlag(META_FLAG::FINISHED))
				m_unfinishedFiles.insert(std::pair<uint64_t, logFile*>(f->getFileId(), f));
		}
		dsOk();
	}


	DLL_EXPORT DS LocalLogFileCache::createNextLogFile(uint64_t srcLogFileId, logFile * &nextFile)
	{
		nextFile = nullptr;
		...
		nextFile = m_current;
		m_lock.lock();
		m_unfinishedFiles.insert(std::pair<uint64_t, logFile*>(nextFile->getFileId(), nextFile));
		m_lock.unlock();
		dsOk();
	}

	DLL_EXPORT DS LocalLogFileCache::finishLogFile(logFile*& file)
	{
		if (m_maxFileSize > 0 || m_maxFileTime > 0)
			dsFailedAndLogIt(1, "can not direct call finishLogFile when file rotate by LocalLogFileCache", ERROR);
		m_lock.lock();
		m_unfinishedFiles.erase(file->getFileId());
		m_lock.unlock();
		file->finish();
		dsOk();
	}

	DLL_EXPORT void LocalLogFileCache::start()
	{
		m_memoryInfo.start();
	...
	DLL_EXPORT void LocalLogFileCache::stop()
	{
		m_memoryInfo.stop();
		m_flushAndPurgeThread.join();
	}

	DLL_EXPORT bool LocalLogFileCache::containSeqNo(uint64_t seqNo)
	{
		RPC::LogSeqNo lsn;
		lsn.seqNo = seqNo;

		if (lsn.fileId > m_current->getFileId() || lsn.fileId < m_begin->getFileId())
			return false;
	}


	DLL_EXPORT LocalLogFileCache::iterator::iterator(LocalLogFileCache * cache) :cache(cache), fileIter(nullptr)
	{
	}

	DLL_EXPORT DS LocalLogFileCache::iterator::seek(uint64_t seqNo, LogEntry * &e)
	{
		RPC::LogSeqNo seq;
		seq.seqNo = seqNo;
	...
	DLL_EXPORT DS LocalLogFileCache::iterator::seekToBegin(LogEntry * &e)
	{
		logFile* currentLog = cache->m_begin;
		if (currentLog == nullptr)
			dsFailedAndLogIt(1, "local log file cache is empty", ERROR);
