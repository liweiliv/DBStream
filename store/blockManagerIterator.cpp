...
namespace STORE
{
	...
	*/
	bool blockManagerIterator::seekByTimestamp(uint64_t timestamp, uint32_t interval, bool equalOrAfter)//timestamp not increase strictly,so we have to check all block
	{
		m_status = UNINIT;
		m_errInfo.clear();
		uint32_t blockId;
	...
	bool blockManagerIterator::seekByLogOffset(uint64_t logOffset, bool equalOrAfter)
	{
		m_status = UNINIT;
		m_errInfo.clear();
	...
	bool blockManagerIterator::seekByRecordId(uint64_t recordId)
	{
		m_status = UNINIT;
		m_errInfo.clear();
		int64_t s = m_manager->m_firstBlockId.load(std::memory_order_relaxed), e = m_manager->m_lastBlockId.load(std::memory_order_relaxed), m;
	...
	blockManagerIterator::blockManagerIterator(uint32_t flag, filter* filter, blockManager* m_manager) :iterator(flag, filter), m_current(nullptr), m_blockIter(nullptr), m_manager(m_manager)
	{
	}
	blockManagerIterator::~blockManagerIterator()
	{
		if (m_blockIter)
			delete m_blockIter;
	}
	iterator::status blockManagerIterator::next()
	{
