...
public:
	...
	*/
	...
	{
		char * indexPos = data,*externPos = data+ keySize*m_keyCount,*externCurretPos = externPos;
		do
		{
			...
			else
			{
				if (k->count < 0x7f)
				{
					*(uint32_t*)(indexPos + keySize) = (k->count << 24) | (externCurretPos - externPos);
				}
				else
				{
					*(uint32_t*)(indexPos + keySize) = (0x80u << 24) | (externCurretPos - externPos);
	...
	template<>
	void createFixedSolidIndex(char * data, iterator<unionKey> &iter, uint16_t keySize)
	{
		char * indexPos = data, *externPos = data + keySize * m_keyCount, *externCurretPos = externPos;
		do
		{
	...
	template<>
	void createVarSolidIndex( char * data, iterator<unionKey> &iter)
	{
		char * indexPos = data, *externPos = data + sizeof(uint32_t) * m_keyCount, *externCurretPos = externPos;
		do
		{
			const keyChildInfo * k = iter.keyDetail();
			*(uint32_t*)indexPos = externCurretPos - externPos;
	...
	template<>
	void createVarSolidIndex( char * data, iterator<binaryType> &iter)
	{
		char * indexPos = data, *externPos = data + sizeof(uint32_t) * m_keyCount, *externCurretPos = externPos;
		do
		{
